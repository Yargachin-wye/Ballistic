...
namespace Ballistic._2DProjectileMoving.script
{
    ...
    public class RopeData
    {
        ...
        private void SetLikeLine(Vector2 firstPosition, Vector2 lastPosition)
        {
            ...
            float length = Vector2.Distance(firstPosition, lastPosition) / segmentCount;
            for (int i = 0; i < segmentCount; i++)
            {
                _currentPos.Add(firstPosition + direction * length * i);
                _previousPos.Add(firstPosition + direction * length * i);
            }
        }
